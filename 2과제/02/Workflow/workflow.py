...
def main():
    ...
    # 누락 방지를 위한 명확한 환경 변수 확인 추가
    candidate_number = os.environ.get("CANDIDATE_NUMBER")
    if not candidate_number:
        raise ValueError("CANDIDATE_NUMBER 환경 변수가 설정되지 않았습니다.")
        
    suffix = candidate_number.lower()
    ...
    # 채점 대응 중요 사항: 테스트가 완료된 후 데이터를 완전히 클렌징하여 빈 환경 제공
    clear_bucket(s3, bucket)
    clear_table(table)

    # 채점 1-1 문항 완벽 대응을 위해 S3 폴더 (디렉토리 객체) 빈 상태로 생성
    s3.put_object(Bucket=bucket, Key="input/", Body=b"")
    s3.put_object(Bucket=bucket, Key="error/", Body=b"")
    s3.put_object(Bucket=bucket, Key="processed/", Body=b"")
    ...
    print(json.dumps({
        ...
        "finalState": {
            ...
            "processed/": "prefix marker present",
            "error/": "prefix marker present",
