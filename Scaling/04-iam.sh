...
SQS_QUEUE_NAME="skills-sqs-queue"
...
KEDA_POLICY_ARN=$(aws iam create-policy \
  --policy-name skills-keda-sqs-policy \
  --policy-document file:///tmp/keda-sqs-policy.json \
  --query 'Policy.Arn' --output text 2>/dev/null || \
  aws iam get-policy \
    --policy-arn "arn:aws:iam::${ACCOUNT_ID}:policy/skills-keda-sqs-policy" \
...
aws iam create-role \
  --role-name skills-keda-sqs-role \
...
KEDA_ROLE_ARN="arn:aws:iam::${ACCOUNT_ID}:role/skills-keda-sqs-role"
...
KARPENTER_POLICY_ARN=$(aws iam create-policy \
  --policy-name skills-karpenter-controller-policy \
  --policy-document file:///tmp/karpenter-policy.json \
  --query 'Policy.Arn' --output text 2>/dev/null || \
  aws iam get-policy \
    --policy-arn "arn:aws:iam::${ACCOUNT_ID}:policy/skills-karpenter-controller-policy" \
    --query 'Policy.Arn' --output text)
...
aws iam create-role \
  --role-name skills-karpenter-controller-role \
